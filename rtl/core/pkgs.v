package proj_pkgs;
   ...
   parameter LAP_BOP_ADR_MSB = LAP_BOP_ADR_LSB + LAP_BOP_CACHE_ADR_BITS -1;

   parameter LAP_COP_CACHE_ADR_BITS = 4; // address row of LAP_N elements
   parameter LAP_COP_ADR_LSB = $clog2(LAP_N);
   parameter LAP_COP_ADR_MSB = LAP_COP_ADR_LSB + LAP_COP_CACHE_ADR_BITS -1;
   ...
   typedef struct packed {
      logic 				 rd;
      logic [LAP_AOP_CACHE_ADR_BITS-1:0] adr;
   } abus_t;

   typedef struct packed {
      logic 				 rd;
      logic [LAP_BOP_CACHE_ADR_BITS-1:0] adr;
   } bbus_t;

   typedef struct packed {
      logic 				 wr;
      logic [LAP_COP_CACHE_ADR_BITS-1:0] adr;
   } cbus_t;
   ...
   typedef struct 		 packed {
      ...
      logic [15:0] 		 cadr;  // partial sum temp storage address
      logic [31:0] 		 sadr;  // store result starting address, for final result storage
